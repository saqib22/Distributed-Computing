...
public class TotalOrderedMulticast {
	...
	public static volatile ArrayList<Process> event_buffer = new ArrayList<>();
	...
	public static Comparator<Process> comp = (event1, event2)->{
	...
	public void manageConnections(){
		...
		try {
			...
			while(events_delivered != NUM_PROCESSES){
				...
				Process event = (Process)obj_ip.readObject();
				clientSocket.close();
				(new Thread(){
					@Override
					public void run(){
	...
	public synchronized void manageBuffers(Process event){
		try {
			if(event.isAck()){
	...
	public void manageAcknowledgements(){
		while(events_delivered != NUM_PROCESSES){
			if(!event_buffer.isEmpty()){
				Process event = event_buffer.get(0);
	...
	public void sendEvent(Process event){
		try {
			if(!event.isAck()){
				logical_time += 1;
			}
	...
	public boolean canWeSendAckToThisGuy(Process event){
		String myEvent = PROCESS_EVENTLIST[pid];
		if(!ack_buffer.isEmpty() && ack_buffer.containsKey(event.getEvent_id())){
			if(ack_buffer.get(event.getEvent_id()).contains(TotalOrderedMulticast.process_id)){
				return false;
			}
		}
		if(event.getPid() == pid){
	...
	public void deliverEvent(Process event){
